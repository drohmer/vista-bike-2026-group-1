"""Test de la chaine geometrique sur donnees synthetiques a verite terrain connue."""
import numpy as np, cv2, math

def Ry(d):  # rotation autour de l'axe vertical (y bas) = changement de cap
    a = math.radians(d); c, s = math.cos(a), math.sin(a)
    return np.array([[c, 0, s], [0, 1, 0], [-s, 0, c]])

def cam_from_azimuth(az_deg):
    """R_wc : monde(Est,Down,Nord) -> camera. Azimut 0=nord, sens horaire."""
    return Ry(-az_deg)
...
print(' azA_vrai azB_vrai | azA_est azB_est | erreur')
bad = 0
for trial in range(8):
    azA = rng.uniform(0, 360); azB = azA + rng.uniform(-50, 50)   # champs qui se recouvrent
    # positions monde : axes (Est, Down, Nord)
    ...
    # points 3D devant les deux cameras
    # points tires DEVANT la camera A puis filtres pour B (evite la boucle infinie)
    P, tries = [], 0
    while len(P) < 400 and tries < 20000:
        tries += 1
        loc = np.array([rng.uniform(-200,200), rng.uniform(-30,10), rng.uniform(30,400)])
        p = RA.T @ loc + CA                      # repere camera A -> monde
        if (RB@(p-CB))[2] > 20: P.append(p)
    if len(P) < 60: print('  (essai ignore: recouvrement insuffisant)'); continue
    P = np.array(P)
    def proj(R, C):
        X = (R@(P-C).T).T
    ...
    # --- la chaine testee (identique a azimuth.py) ---
    ...
    print('  %6.1f %6.1f | %6.1f %6.1f | %5.1f %5.1f %s'
          %(azA,azB,eA,eB,errA,errB,'  <-- FAUX' if max(errA,errB)>5 else ''))
print('\n%d/8 essais faux' % bad)
